    ...
    onFan(t){
        if(t===1){
            ...
            arr.forEach(type=>{
                let data = this.state[type];
                this.setState({[type]:data.sort((a,b)=>{
                    let lb11 = this.compute(5,20,a["lb"])
                    let lb22 = this.compute(5,20,b["lb"])
                    let hs1 = this.compute(5,10,a['hsl'])
                    let hs2 = this.compute(5,10,b['hsl'])
                    let n1 = lb11 + hs1
                    let n2 = lb22 + hs2;
                    if(isNaN(n1))n1=0;
                    if(isNaN(n2))n2=0;
    ...
    render(){
        return (
            <div>
                ...
                <div><button className="btn" onClick={this.onFan.bind(this,1)}>方案一</button></div>
                ...
                <table className="list">
                    ...
                    <tbody>
                    {
                        this.state.owner.map((item,index)=>{
                            return (
                                <Item action={this.action.bind(this,'owner')} hasOwner={true} key={item.code} index={index} data={item} onUpdate={this.onUpdate.bind(this,index,'owner')}/>
                            )
                        })
                    }
                    </tbody>
